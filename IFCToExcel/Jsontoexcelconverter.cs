...
namespace IfcToExcelWinForms
{
    ...
    public static class JsonToExcelConverter
    {
        ...
        public static void Convert(string jsonPath, string templateXlsxPath, string outXlsxPath)
        {
            ...
            // When multiple beams are marked isBearingMember (e.g. column + framing beams),
            // prefer the non-IPar member (column) since IPar beams are typically the framing
            // members that connect TO the column.
            int bearingBeamId = -1;
            var bearingCandidates = new List<(int Id, string CsType)>();
            var angleBeams = new List<JsonElement>();
            foreach (var beam in root.GetProperty("beams").EnumerateArray())
            {
                if (beam.GetProperty("isBearingMember").GetBoolean())
                    bearingCandidates.Add((beam.GetProperty("id").GetInt32(),
                        beam.GetProperty("crossSectionType").GetString() ?? ""));
                var csType = beam.GetProperty("crossSectionType").GetString() ?? "";
                if (csType.Equals("RolledAngle", StringComparison.OrdinalIgnoreCase))
                    angleBeams.Add(beam);
            }
            if (bearingCandidates.Count == 1)
            {
                bearingBeamId = bearingCandidates[0].Id;
            }
            else if (bearingCandidates.Count > 1)
            {
                // Prefer non-IPar bearing member (the column, typically RolledRhsPar / RolledChs / etc.)
                var nonIPar = bearingCandidates
                    .Where(b => !b.CsType.Equals("RolledIPar", StringComparison.OrdinalIgnoreCase))
                    .ToList();
                bearingBeamId = nonIPar.Count > 0 ? nonIPar[0].Id : bearingCandidates[0].Id;
            }
            ...
            foreach (var grid in root.GetProperty("boltGrids").EnumerateArray())
            {
                ...
                {
                    // Use bolt world position to classify the face.
                    // Web plate normals (ClassifyFaceFromPlate) can be unreliable because
                    // I-beam web plates have normals along the web which may not correspond
                    // to the column face the connection is on.
                ...
                {
                    // Key by bolt world position face (most reliable for angle grids)
                    // because the angle plate normal can point toward or away from the
                    // column depending on the angle's orientation, and the angle beam's
                    // first plate may be the horizontal leg with a Z-axis normal.
                    string angleFace = ClassifyGridFace(wps[0]);
                    if (!angleGridByFace.ContainsKey(angleFace))
                    {
                        angleGridByFace[angleFace] = new AngleGridInfo
                        {
                            Positions = wps,
                            PlateId = anglePid.Value,
                        };
                    }
                    // Also update the angle beam's face mapping to match the bolt position
                    if (plateToAngleBeamId.TryGetValue(anglePid.Value, out int abId))
                        angleBeamFace[abId] = angleFace;
                }
            }
        ...
        {
            // Angle beams have two plates: a horizontal leg (axisZ ≈ (0,0,±1))
            // and a vertical leg. Skip horizontal plates and use the vertical plate's
            // origin position to determine which column face the angle sits on.
            // The origin of the vertical plate is the most reliable indicator when
            // the coordinate perpendicular to the face is dominant.
            // However, in cases where the angle is far from the column center along
            // the beam axis, the origin position can be misleading.
            // This method is used only as a fallback; the primary classification
            // is now done via bolt grid world positions (see step 4).
            foreach (var plate in angleBeam.GetProperty("plates").EnumerateArray())
            {
                if (plate.TryGetProperty("axisZ", out var az))
                {
                    double azZ = az.TryGetProperty("z", out var zEl) ? zEl.GetDouble() : 0.0;
                    if (Math.Abs(azZ) > 0.5) continue; // skip horizontal plate
